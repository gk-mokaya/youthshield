...
def manage_backups(request):
    ...
    try:
        backup_job = BackupJob.objects.get(name='Auto Backup')
        auto_backup = {
            'frequency': backup_job.frequency,
            'time': backup_job.backup_time.strftime('%H:%M'),
            'max_backups': backup_job.max_backups,
            'is_active': backup_job.is_active,
            'next_run': backup_job.next_run,
            'last_run': backup_job.last_run,
        }
    ...
    backup_logs = BackupLog.objects.all()[:10]  # Last 10 logs
...
def save_auto_backup_settings(request):
    if request.method == 'POST':
        try:
            ...
            backup_time_str = request.POST.get('time')
            max_backups = request.POST.get('max_backups')

            # Validate required fields
            if not all([frequency, backup_time_str, max_backups]):
            ...
            # Get or create the backup job (assuming one main job for auto backup)
            ...
            if not created:
                # Update existing job
                backup_job.frequency = frequency
                backup_job.backup_time = backup_time
                backup_job.max_backups = int(max_backups)
                backup_job.save()
            ...
            BackupLog.objects.create(
                job=backup_job,
                action='scheduled',
                message=f'Auto backup settings updated: {frequency} at {backup_time_str}, max {max_backups} backups',
                success=True
            )
            ...
            return JsonResponse({'success': False, 'error': f'Invalid time format. Please use HH:MM format: {str(e)}'})
        except Exception as e:
            return JsonResponse({'success': False, 'error': str(e)})
    return JsonResponse({'success': False})
